...
public class IndividualArtistController {
    ...
    public void showArtistList(String artistName) throws ClassNotFoundException, SQLException {
        ...
        String query2 = "SELECT Song_Name,Artist FROM MainTable Group by Song_Name Order by Song_Name" ;
        PreparedStatement pre = connect.prepareStatement(query2);
        ResultSet result = pre.executeQuery();
        while (result.next()) {
            String songName = result.getString("Song_Name");
            String artist = result.getString("Artist");
            if (artist!=null && artist.contains(artistName))
                artistList.getItems().add(songName);
        }
    ...
    @FXML
    void playArtistList(MouseEvent event) throws ClassNotFoundException, SQLException {
        String songUrl=null;
        controller.songName= (String) artistList.getSelectionModel().getSelectedItem();
        Class.forName("com.mysql.cj.jdbc.Driver");
        String url = "jdbc:mysql://localhost:3306/media";
        Connection connect = DriverManager.getConnection(url, "root", "utkarshsql");
        String query = "SELECT SongUrl FROM MainTable WHERE Song_Name = '"+controller.songName+"' GROUP BY SongUrl ;";
        PreparedStatement pre = connect.prepareStatement(query);
        ResultSet result = pre.executeQuery();
        while(result.next()){
            songUrl=result.getString("SongUrl");
        }
    ...
    public void showArtistName(String artist) {
